...
function RescheduleContent() {
    ...
    try {
      ...
      if (isMockUser) {
        ...
        if (mockAppt) {
          ...
          const timeStr = format(new Date(mockAppt.appointmentStartTime * 1000), 'HH:mm');
        ...
        const timeStr = format(new Date(data.appointmentStartTime * 1000), 'HH:mm');
        setSelectedTimeSlot(timeStr);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isMockUser) {
        // Mock reschedule - just show success
        setTimeout(() => {
          setShowSuccess(true);
          setSubmitting(false);
        }, 1000);
      } else {
        // TODO: Integrate with reschedule API endpoint
        // PATCH /api/appointments/:id/reschedule
        // Body: { appointmentStartTime, appointmentEndTime }
  ...
  const canReschedule = () => {
    if (!appointment || !selectedDate || !selectedTimeSlot) return false;
    ...
    return hoursUntilAppointment >= 24; // Can reschedule if more than 24 hours away
  };
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                <p className={`font-semibold ${
                  canRescheduleNow ? 'text-blue-900' : 'text-yellow-900'
                }`}>
                  {canRescheduleNow 
                ...
                <p className={`text-sm mt-1 ${
                  canRescheduleNow ? 'text-blue-800' : 'text-yellow-800'
                }`}>
                  {canRescheduleNow
          ...
          {canRescheduleNow ? (
            <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-lg p-8">
              ...
              <p className="text-xs text-gray-500 text-center mt-4">
                TODO: Integrate with reschedule API endpoint when available
              </p>
            </form>
          ) : (
