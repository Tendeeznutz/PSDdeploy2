...
function BookingDetailsContent() {
  ...
  const formatDate = (timestamp: number) => format(new Date(timestamp * 1000), 'MMMM d, yyyy');
          ...
          {showSuccess && (
            <div className="mb-6 p-6 bg-green-50 border-2 border-green-200 rounded-lg">
              <div className="flex items-start">
                ...
                <button onClick={() => setShowSuccess(false)} className="text-green-600 hover:text-green-700 ml-4">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 md:p-8 mb-6">
                      ...
                      return (
                        <div key={step.id} className="relative flex items-start">
                          ...
                          <div className="ml-4 flex-1">
                            ...
                            {isActive && <p className="text-xs text-primary-600 mt-1">Current status</p>}
                            {isCompleted && <p className="text-xs text-gray-500 mt-1">Completed</p>}
                            {!isActive && !isCompleted && <p className="text-xs text-gray-400 mt-1">Upcoming</p>}
                          </div>
                        </div>
                      );
                    })}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <p className="text-sm text-gray-600">{formatDate(appointment.appointmentStartTime)}</p>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <p className="text-sm text-gray-600">{appointment.customer?.customerAddress || 'N/A'}</p>
                <p className="text-sm text-gray-600">Singapore {appointment.customer?.customerPostalCode || ''}</p>
              </div>
