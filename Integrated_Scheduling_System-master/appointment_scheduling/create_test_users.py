...
Newly created users use password: password123
"""
...
def log_user_result(created, label):
    if created:
        print(f"[OK] Created: {label}")
    else:
        print(f"[OK] Skipping existing user: {label}")
...
def create_test_data():
    ...
    log_user_result(
        created, f"{coordinator1.coordinatorName} - {coordinator1.coordinatorEmail}"
    )
    ...
    log_user_result(
        created, f"{coordinator2.coordinatorName} - {coordinator2.coordinatorEmail}"
    )
    ...
    log_user_result(
        created, f"{technician1.technicianName} - {technician1.technicianEmail}"
    )
    ...
    log_user_result(
        created, f"{technician2.technicianName} - {technician2.technicianEmail}"
    )
    ...
    log_user_result(
        created, f"{technician3.technicianName} - {technician3.technicianEmail}"
    )
    ...
    log_user_result(created, f"{customer1.customerName} - {customer1.customerEmail}")
    ...
    log_user_result(created, f"{customer2.customerName} - {customer2.customerEmail}")
    ...
    log_user_result(created, f"{customer3.customerName} - {customer3.customerEmail}")
    ...
    log_user_result(created, f"{customer4.customerName} - {customer4.customerEmail}")
    ...
    for brand, models in brands:
        for model in models:
            aircon, created = AirconCatalogs.objects.get_or_create(
                airconBrand=brand, airconModel=model
            )
            print(
                f"[OK] {'Created' if created else 'Already exists'}: {brand} - {model}"
            )
