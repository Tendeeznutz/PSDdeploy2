...
def get_available_time_slots(technician_id, date_str, duration_hours=1):
    ...
    work_start = datetime.combine(
        target_date, datetime.min.time().replace(hour=start_hour, minute=start_minute), tzinfo=SGT
    )
    work_end = datetime.combine(
        target_date, datetime.min.time().replace(hour=end_hour, minute=end_minute), tzinfo=SGT
    )
