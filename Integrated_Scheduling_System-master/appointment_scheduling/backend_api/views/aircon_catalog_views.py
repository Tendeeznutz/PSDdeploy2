...
class AirconCatalogViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        if request.query_params.get("airconBrand") is not None:
            queryset = AirconCatalogs.objects.filter(
                airconBrand__icontains=request.query_params.get("airconBrand")
            )
        elif request.query_params.get("airconModel") is not None:
            queryset = AirconCatalogs.objects.filter(
                airconModel__icontains=request.query_params.get("airconModel")
            )
        elif request.GET:
